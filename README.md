...
`sync` полностью пересобирает `content/pages/**/*.html`, `content/images/**` и
`content/manifest.json` с нуля и печатает отчёт (добавлено/изменено/убрано/предупреждения).
После этого обычный `git diff` в `content/` показывает, что реально изменилось.
